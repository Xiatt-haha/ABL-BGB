...
from dataset import DocTamperDataset, RSCDataset,TrainDocTamperDataset
...
parser.add_argument('--pth', type=str, default='outputs/BGDB/resnet18/ckpt/checkpoint-latest.pth')
parser.add_argument('--lmdb_name', type=str, default='DocTamperV1-TestingSet')
...
if use_db:
    ...
    n_class = 2
model = seg_dtd('',n_class).cuda()
model = torch.nn.DataParallel(model)
log_filename = f'outputs/BGDB/resnet18/eval_{args.lmdb_name}_{args.minq}.log'
...
def eval_net_dtd(model, test_data, plot=False,device='cuda'):
    train_loader1 = DataLoader(dataset=test_data, batch_size=12, num_workers=0, shuffle=False)
    ...
    with torch.no_grad():
        ...
        #print('[val] iou:{} pre:{} rec:{} f1:{}'.format(iu[1],precisons,recalls,(2*precisons*recalls/(precisons+recalls+1e-8))))
        logger.info('[val] iou:{} pre:{} rec:{} f1:{}'.format(iu[1],precisons,recalls,(2*precisons*recalls/(precisons+recalls+1e-8))))
